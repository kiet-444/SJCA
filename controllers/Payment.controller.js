...
const checkSumkey = process.env.PAYOS_ENVIRONMENT || "";

const generatePayOSSignature = (data, key) => {
  return crypto
...
const createPaymentService = async (req, res) => {
  try {
    ...
    const data = `amount=${response.amount}&cancelUrl=${response.cancelUrl}&description=${response.description}&orderCode=${response.orderCode}&returnUrl=${response.returnUrl}`;
  ...
  try {
    ...
    if (!jobGroup || jobGroup.userId !== userId) {
      return res.status(404).json({
        success: false,
        message: "JobGroup không tồn tại hoặc không thuộc người dùng",
      });
    }
    ...
    const response = {
      ...
      webhookUrl:
        "https://sjcp-fha4a5e8f6arc7cg.eastasia-01.azurewebsites.net/api/payment/callback",
    };
    ...
    res
      .status(500)
      .json({ success: false, message: "Lỗi tạo thanh toán JobGroup" });
  }
};
...
const releasePayment = async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(userIds) || userIds.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Danh sách người dùng không hợp lệ" });
    }
    ...
    if (users.length !== userIds.length) {
      return res.status(404).json({
        success: false,
        message: "Một hoặc nhiều người dùng không tìm thấy",
      });
    }
    ...
    if (
      !jobPosting ||
      jobPosting.status !==
        "completed" /*|| jobPosting.number_of_person !== userIds.length*/
    ) {
      return res
        .status(404)
        .json({ success: false, message: "Công việc không hợp lệ" });
    }

    const escrowWalletEmployer = await EscrowWallet.findOne({
    ...
    if (!escrowWalletEmployer) {
      return res.status(404).json({
        success: false,
        message: "Escrow Wallet của nhà tuyển dụng không tìm thấy",
      });
    }
    ...
    for (const user of users) {
      const jobExecutes = await JobExecute.findAll({
        where: { jobPostingId, userId: user.id },
        Transaction,
      });
      ...
      if (jobExecutes.length === 0) {
        return res.status(400).json({
          success: false,
          message: `Không có công việc thực hiện bởi người dùng ${user.id}`,
        });
      }
      ...
      const amountToTransfer = parseFloat(
        ((totalProgressComplete / 100) * jobPosting.salary).toFixed(2)
      );
    ...
    if (escrowWalletEmployer.balance < totalAmountToDeduct) {
      ...
      return res.status(400).json({
        success: false,
        message: "Số dư không đủ trong Escrow Wallet",
      });
    }
    ...
    if (employer) {
      await transporter.sendMail({
        ...
        html: `
                  <p>Bạn sắp thực hiện thanh toán cho công việc "${
                    jobPosting.title
                  }".</p>
                  <p>Thanh toán:</p>
                  <ul>
    ...
    return res
      .status(200)
      .json({ success: true, message: "Giải phóng tiền thành công" });
  } catch (error) {
    // await Transaction.rollback();
    console.error(error);
    return res
      .status(500)
      .json({ success: false, message: "Lỗi giải phóng thanh toán" });
  }
};
...
const paymentHistory = async (req, res) => {
  try {
    ...
    if (!payments || payments.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "The user has no payment history" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Wrong when get payment history" });
  }
};
