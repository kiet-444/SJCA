...
const CVManagementController = {
  ...
  async setDefaultCV(req, res) {
    try {
      ...
      if (!userId || !cvId) {
        return res
          .status(400)
          .json({ message: "userId and cvId are required" });
      }
      ...
      await CV.update({ status: "Default" }, { where: { id: cvId, userId } });
  ...
  async applyForJob(req, res) {
    try {
      const { jobPostingId, cvId, coverLetter } = req.body;
      ...
      if (!job || !cv) {
        return res
          .status(404)
          .json({ message: "Người dùng, công việc hoặc CV không tồn tại." });
      }
      ...
      if (existingApplication) {
        return res
          .status(400)
          .json({ message: "Bạn đã ứng tuyển công việc này bằng CV này." });
      }
  ...
  async getApplicationsByCV(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Đã xảy ra lỗi khi lấy danh sách đơn ứng tuyển." });
    }
  },
};
