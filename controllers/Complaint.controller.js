...
const ComplaintRecordController = {
    createComplaintRecord: async (req, res) => {
        try {
            ...
            const { jobPostingId, description, type } = req.body;
            
            const newComplaintRecord = await ComplaintRecord.create({ jobPostingId, userId, description, type });
    ...
    updateComplaintRecord: async (req, res) => {
        try {
            const { id } = req.params;
            const { jobPostingId, userId, content } = req.body;
            const complaintRecord = await ComplaintRecord.findByPk(id);
            if (!complaintRecord) {
                return res.status(404).json({ message: 'Complaint record not found' });
            }
            await complaintRecord.update({ jobPostingId, userId, content });
